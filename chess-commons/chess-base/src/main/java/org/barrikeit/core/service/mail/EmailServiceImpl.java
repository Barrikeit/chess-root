...
public class EmailServiceImpl implements EmailService {
  ...
  @Value("${mail.from:no-reply@ajedrezillo.es}")
  ...
  public String getMailSubject(MailTipo mailTipo) {
    return switch (mailTipo) {
      case CREATE_USER -> messageSource.getMessage(
          "mail.subject.createUser", null, LocaleContextHolder.getLocale());
      case VERIFY_USER -> messageSource.getMessage(
          "mail.subject.verifyUser", null, LocaleContextHolder.getLocale());
      case BAN_USER -> messageSource.getMessage(
          "mail.subject.banUser", null, LocaleContextHolder.getLocale());
      case UPDATE_USER -> messageSource.getMessage(
